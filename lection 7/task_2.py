...
class PersonInfo:
    def __init__(self, full_name, age, *dev):
    ...
    def new_salary(self):
        ...
        top_three_letters = [x[0] for x in sorted_letters[:3]]

        count = 0
        for word in self.dev:
            for letter in word:
                if letter in top_three_letters:
                    count += 1

        return 1337 * self.age * count
